...
public class LabyrinthTempInit : MonoBehaviour {
    ...
    private static LabyrinthTempInit instance;
    public static LabyrinthTempInit Instance
    {
        get
        {
            return instance;
    ...
    void Start()
    {
        ...
        ground.transform.position = new Vector3((float)17.37536,(float)0,(float)17.60871);
        ground.transform.parent = transform;
        ground.transform.localScale = new Vector3((float) 3.47158, (float) 1.0, (float) 3.475);
        ...
        for (int i = 0; i <= Maze.GetUpperBound(0); i++)
        {
            for (int j = 0; j <= Maze.GetUpperBound(1); j++)
            {
                if (Maze[i, j] == 1)
                {
                    ...
                    NavMeshModifier modifier = ptype.AddComponent<NavMeshModifier>();
